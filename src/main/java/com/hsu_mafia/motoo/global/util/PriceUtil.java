...
import com.hsu_mafia.motoo.api.kis.response.CurrentPriceRes;
import com.hsu_mafia.motoo.api.kis.response.MinutePriceRes;
import com.hsu_mafia.motoo.api.kis.response.PeriodPriceRes;
...
import com.hsu_mafia.motoo.api.stock.response.DayWeekRes;
import com.hsu_mafia.motoo.api.stock.response.MinuteRes;
import com.hsu_mafia.motoo.api.stock.response.StockRankRes;
import com.hsu_mafia.motoo.api.stock.response.StockVolumeRes;
import com.hsu_mafia.motoo.api.stock.response.UserValueRes;
...
public class PriceUtil {
    ...
    private void calculateTradingVolume(Integer nextState) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            StockEntity stock = stockVolumeResList.get(i).getStock();
    ...
    public UserValueRes calculateUserValue(Long userId) {
        UserEntity user = userRepository.findById(userId).orElseThrow(MySQLSearchException::new);
        Long stockValue = 0L;

        List<UserStockEntity> userStockList = userStockRepository.findAllByUserAndBankruptcyNo(user,
